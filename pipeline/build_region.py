...
def _attach_area_overrides(con):
    """Load data/build/area-overrides.json into area_extra.

    One row per (county, city boundary name, ISD boundary name) area, carrying
    the rate the map is short by and the units it comes from. Generated by
    area_overrides.py from CAD-published area totals; absent that file this is
    a no-op, so the build still runs on a clean checkout.
    """
    import json as _json

    con.execute(
        """CREATE OR REPLACE TABLE area_extra(
           county TEXT, city_name TEXT, isd_name TEXT,
           extra_rate DOUBLE, extra_names TEXT)"""
    )
    path = BUILD / "area-overrides.json"
    if not path.exists():
        print("area overrides: none (run area_overrides.py to generate)")
        return
    rows, seen = [], {}
    for r in _json.loads(path.read_text()):
        names = "; ".join(f"{u['name']}={u['rate']}" for u in r["units"])
        for isd in r.get("isd_names") or []:
            for city in r.get("city_names") or [None]:
                key = (r["county"], city, isd)
                if key in seen:
                    # Two published areas resolving to the same (city, ISD) key
                    # would multiply rows in the LEFT JOIN and duplicate parcels
                    # in the tiles. Never let that through silently.
                    raise SystemExit(
                        f"area overrides: {key} appears twice "
                        f"({seen[key]} vs {r['area']}) — fix area_overrides.py")
                seen[key] = r["area"]
                rows.append((r["county"], city, isd, r["extra_rate"], names))
    con.executemany("INSERT INTO area_extra VALUES (?,?,?,?,?)", rows)
    n = con.execute(
        """SELECT count(*) FROM parcels_all p JOIN area_extra a
           ON a.county = p.county
          AND a.isd_name IS NOT DISTINCT FROM p.isd_name
          AND a.city_name IS NOT DISTINCT FROM p.city_name""").fetchone()[0]
    counties = sorted({r[0] for r in rows})
    print(f"area overrides: {len(rows)} areas across {len(counties)} counties "
          f"({', '.join(counties)}); {n:,} parcels corrected")
...
def _attach_rates_and_export(con, city_rates, isd_rates, wd_rates=None):
    ...
    # ── area corrections proven from a CAD's own published area totals.
    # Where a county prints the combined rate for each of its areas, that total
    # decomposes to exactly one unit set (area_overrides.py), which names a
    # district the spatial join misses for a specific city/ISD area — most often
    # an ESD, whose boundaries are published nowhere statewide.
    _attach_area_overrides(con)
    ...
    con.execute(
        ...
        SELECT p.uid, p.prop_id, p.addr, p.mkt, p.county, p.city_name, p.isd_name,
               ...
               coalesce(rr.nominal_rate,
                        p.base + coalesce(c.rate, 0) + coalesce(i.rate, 0)
                          + coalesce(s.special_rate, 0)
                          + coalesce(ax.extra_rate, 0)) AS nominal_rate,
               p.geom
        FROM parcels_all p
        ...
        -- area corrections apply only where the roll doesn't already give the
        -- exact stack; NULL city means "outside any city", which is a real
        -- area in the published totals, so match it with IS NOT DISTINCT FROM
        LEFT JOIN area_extra ax ON ax.county = p.county
             AND ax.isd_name IS NOT DISTINCT FROM p.isd_name
             AND ax.city_name IS NOT DISTINCT FROM p.city_name"""
    )
            ...
            f"""COPY (
              SELECT p.prop_id AS id, p.addr, p.mkt::BIGINT AS mkt, round(p.nominal_rate, 4) AS rate,
                     ...
                     round(CASE WHEN rr.prop_id IS NOT NULL THEN 0
                                ELSE coalesce(s.special_rate, 0)
                                     + coalesce(ax.extra_rate, 0) END, 4) AS sd,
                     coalesce(rr.stack,
                              nullif(concat_ws('; ', s.special_names,
                                               ax.extra_names), '')) AS sdn,
